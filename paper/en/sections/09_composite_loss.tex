...
\aletheion{} uses a composite loss with 13 components, grouped into
standard cross-entropy and 12 epistemic losses with progressive
annealing.
...
\midrule
1 & VARO & 0.01 & Regularize conditional variance $q_1 \cdot q_2$ \\
2 & VI & 0.01 & Maintain $\phi \geq \phi_{\min}$ \\
3 & MAD Calibration & 0.01 & Calibrate confidence vs accuracy \\
...
\paragraph{VARO (Conditional Variance).}
Regularizes the interaction between both uncertainties:

\begin{equation}
    \calL_{\text{VARO}} = \E\big[(q_1 \cdot q_2 - \bar{q}_1 \cdot \bar{q}_2)^2\big]
\end{equation}

This forces $q_1$ and $q_2$ to capture complementary information,
avoiding redundancy.

\paragraph{VI Regularization.}
Penalizes degraded manifold:
