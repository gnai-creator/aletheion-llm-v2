...
\textbf{350M} & \textbf{1024} & \textbf{16} & \textbf{24} & \textbf{4096} & \textbf{{\raise.17ex\hbox{$\scriptstyle\sim$}}354M} & \textbf{4$\times$ H100 / 1$\times$ RTX 4090} \\
...
\subsection{354M Two-Phase Training}

Training the 354M model follows a two-phase strategy:

\paragraph{Phase 1: Backbone (4$\times$ H100 80GB).}
The backbone is trained with DDP (Distributed Data Parallel) on 4$\times$ H100 SXM 80GB
GPUs using cross-entropy and STP losses only (tomography disabled for throughput).
The configuration uses batch size 16 per GPU, effective batch 64, bf16 mixed precision,
without gradient checkpointing (sufficient VRAM):

\begin{table}[H]
\centering
\caption{Phase 1 training configuration (4$\times$ H100).}
\label{tab:phase1}
\begin{tabular}{lr}
\toprule
...
Throughput & {\raise.17ex\hbox{$\scriptstyle\sim$}}150K tokens/s \\
Total tokens & 7B \\
Estimated time & {\raise.17ex\hbox{$\scriptstyle\sim$}}13 hours \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Phase 2: Epistemic Fine-Tuning (1$\times$ RTX 4090).}
After the backbone converges, the full epistemic system (14 losses with annealing)
is activated on a single RTX 4090 (24GB VRAM) with conservative hyperparameters:
learning rate 3$\times$ reduced, epistemic lambdas at 10\% of defaults with slow
annealing (30\% warmup, 80\% ramp), and EWC to protect the backbone.

\begin{table}[H]
\centering
\caption{Memory estimate for Phase 2 on RTX 4090.}
\label{tab:memory}
\begin{tabular}{lr}
\toprule
...
Epistemic tomography & {\raise.17ex\hbox{$\scriptstyle\sim$}}8--12 GB \\
PyTorch overhead & {\raise.17ex\hbox{$\scriptstyle\sim$}}1.5 GB \\
\midrule
\textbf{Total} & \textbf{{\raise.17ex\hbox{$\scriptstyle\sim$}}17--22 GB / 24 GB} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Tokens and Time:}
Phase 1 trains on 7B tokens (Chinchilla-optimal for 354M \cite{hoffmann2022chinchilla}),
...
\begin{equation}
    \text{time (Phase 1)} \approx \frac{7 \times 10^9}{150000 \text{ tok/s}} \approx 13 \text{ hours}
\end{equation}

Phase 2 fine-tunes on 1B tokens ({\raise.17ex\hbox{$\scriptstyle\sim$}}14\% of the dataset)
at {\raise.17ex\hbox{$\scriptstyle\sim$}}8--12K tokens/s on a single RTX 4090,
estimated at {\raise.17ex\hbox{$\scriptstyle\sim$}}17--34 hours.
