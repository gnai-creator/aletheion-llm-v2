...
onde $d = d_{\text{model}}$ \'e a dimens\~ao do modelo. O \aletheion{} utiliza
...
\paragraph{FeedForward com SwiGLU.}
A camada feedforward utiliza a ativa\c{c}\~ao SwiGLU \cite{shazeer2020glu}:

\begin{equation}
    \text{FFN}(\bx) = (\text{Swish}(\bx \bm{W}_{\text{gate}}) \odot \bx \bm{W}_{\text{up}}) \bm{W}_{\text{down}}
\end{equation}

onde $\bm{W}_{\text{gate}}, \bm{W}_{\text{up}} \in \R^{d \times d_{\text{ff}}}$ e
...
\begin{table}[H]
\centering
\caption{Configura\c{c}\~ao do modelo 354M (RTX 4090).}
