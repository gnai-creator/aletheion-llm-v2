...
Os gates $q_1$ e $q_2$ s\~ao implementados como MLPs compactos com
profundidade configur\'avel (default 2 camadas, dimens\~ao oculta 64):

\begin{equation}
    \text{QGate}(\bh) = \sigma\big(\bm{W}_2 \cdot \text{ReLU}(\bm{W}_1 \bh + \bm{b}_1) + \bm{b}_2\big)
\end{equation}

com $\bm{W}_1 \in \R^{64 \times d}$ e $\bm{W}_2 \in \R^{1 \times 64}$.

\subsection{Temperatura Adaptativa}

A temperatura de amostragem \'e controlada adaptativamente pelo sinal
combinado de incerteza $c = \text{clamp}(q_1 \cdot q_2, \epsilon, 1)$:

\begin{equation}
    \tau = \begin{cases}
        \tau_{\text{base}} / c & \text{se } c < \tau_{\text{threshold}} \\
        \tau_{\text{base}} & \text{caso contr\'ario}
    \end{cases}
\end{equation}

onde $\tau_{\text{base}} = 1.0$ e $\tau_{\text{threshold}} = 0.5$.
Quando ambas as incertezas s\~ao altas (produto excede o threshold),
a temperatura sobe inversamente, criando distribui\c{c}\~oes mais uniformes.
Caso contr\'ario, a temperatura base \'e mantida.
