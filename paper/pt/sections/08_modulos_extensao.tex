...
onde $g = \sigma(\bm{w}^\top [\bh; \text{MLP}(\bm{z})])$ \'e um gate
aprendido. O \texttt{PolicyBinding} mapeia estados para par\^ametros
de gera\c{c}\~ao (temperatura, \textit{max\_tokens}).
