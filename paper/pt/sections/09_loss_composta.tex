...
O \aletheion{} utiliza uma loss composta com 13 componentes, agrupadas em
cross-entropy padr\~ao e 12 losses epist\^emicas com \textit{annealing}
progressivo.

\subsection{Formula\c{c}\~ao Geral}

\begin{equation}
    \calL_{\text{total}} = \lambda_{\text{CE}} \cdot \calL_{\text{CE}} + \alpha(t) \cdot \sum_{i=1}^{12} \lambda_i \cdot \calL_i
    \label{eq:total_loss}
\end{equation}
...
\midrule
1 & VARO & 0.01 & Regulariza vari\^ancia condicional $q_1 \cdot q_2$ \\
2 & VI & 0.01 & Mant\'em $\phi \geq \phi_{\min}$ \\
3 & MAD Calibra\c{c}\~ao & 0.01 & Calibra confian\c{c}a vs acur\'acia \\
...
\paragraph{VARO (Vari\^ancia Condicional).}
Regulariza a intera\c{c}\~ao entre as duas incertezas:

\begin{equation}
    \calL_{\text{VARO}} = \E\big[(q_1 \cdot q_2 - \bar{q}_1 \cdot \bar{q}_2)^2\big]
\end{equation}

Isso for\c{c}a $q_1$ e $q_2$ a capturarem informa\c{c}\~oes complementares,
evitando redund\^ancia.

\paragraph{Regulariza\c{c}\~ao VI.}
Penaliza manifold degradado:

\begin{equation}
    \calL_{\text{VI}} = \E\big[\text{ReLU}(\phi_{\min} - \phi)^2\big], \quad \phi_{\min} = 0.3
\end{equation}
